...
COPY arellan-frontend-web/package.json arellan-frontend-web/package-lock.json ./arellan-frontend-web/
...
WORKDIR /workspace/arellan-frontend-web
...
RUN npm install --no-audit --no-fund \
 ...
 && mkdir -p $UI/@types \
 && ln -s /workspace/arellan-frontend-web/node_modules/react $UI/react \
 && ln -s /workspace/arellan-frontend-web/node_modules/react-dom $UI/react-dom \
 && ln -s /workspace/arellan-frontend-web/node_modules/@types/react $UI/@types/react \
 && ln -s /workspace/arellan-frontend-web/node_modules/@types/react-dom $UI/@types/react-dom

COPY arellan-frontend-web/. .
RUN npm run build
...
COPY --from=builder /workspace/arellan-frontend-web/public ./public
COPY --from=builder /workspace/arellan-frontend-web/.next/standalone ./
COPY --from=builder /workspace/arellan-frontend-web/.next/static ./.next/static
